...
void saisir_agent(agent *a)
{
	...
	printf("donner la fonction de l'agent \n");
	scanf("%s",a->fonction_agent);
...
void ajout_agent(char *agents)
{
   ...
   agent a1;
    ...
    agent a; 
 while (fscanf(f, "%s %s %s %s %f %s %s %s %s %s", a.id_agent, a.nom_agent, a.prenom_agent,a.sexe_agent, &a.sal_agent, a.type_contact, a.contact,a.moyen_agent,a.park,a.fonction_agent)!= EOF) 
	{
...
fprintf(f, "%s %s %s %s %.2f %s %s %s %s %s \n", a1.id_agent, a1.nom_agent,a1.prenom_agent,a1.sexe_agent,a1.sal_agent,a1.type_contact,a1.contact,a1.moyen_agent,a1.park,a1.fonction_agent);
...
void affichage_agent (agent *a,char *agents)
{
    ...
    while (fscanf(f, "%s %s %s %s %f %s %s %s %s %s", a->id_agent, a->nom_agent, a->prenom_agent,a->sexe_agent, &a->sal_agent, a->type_contact, a->contact,a->moyen_agent,a->park,a->fonction_agent) !=EOF) 
	{
        printf("%s %s %s %s %.2f %s %s %s %s %s\n", a->id_agent, a->nom_agent, a->prenom_agent,a->sexe_agent, a->sal_agent, a->type_contact, a->contact,a->moyen_agent,a->park,a->fonction_agent); 
    	}
		...
		pos=1;
                ...
                switch (x) {
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        printf("Saisir la modification du fonction: \n");
                        scanf("%s", a.fonction_agent);
                        break;
                    default : 
		 	...
		 	fprintf(f1, "%s %s %s %s %.2f %s %s %s %s %s\n", a.id_agent, a.nom_agent, a.prenom_agent,a.sexe_agent, a.sal_agent, a.type_contact, a.contact,a.moyen_agent,a.park,a.fonction_agent);
	...
	void affichage_res(char *reservations)
		{
			...
			while (fscanf(f, "%s %s %s %s %s %s %d %d %d %d %f", r.id_reservation, r.id_citoyen, r.id_parking,r.type_veh, r.services_com, r.temps, &r.nb_voi, &r.date_res.j, &r.date_res.m,
                   &r.date_res.a, &r.total) !=EOF) 
					{
					if(r.date_res.j==j1 && r.date_res.m==m1 && r.date_res.a==a1)
					{
        				printf("%s %s %s %s %s %s %d %d %d %d %.2f \n", r.id_reservation, r.id_citoyen, r.id_parking,r.type_veh, r.services_com, r.temps, r.nb_voi, r.date_res.j, r.date_res.m,
                   r.date_res.a, r.total);
					trouve=1; 
					}
    					}
			if(trouve==0)
